...
def main():
    ...
    print("Running inference via official LabLensPipeline...")
    try:
    ...
    img_yolo = img.copy()
    for d in detections:
        # Pipeline returns DetectionPosition with normalized coords. Need to convert back to pixels!
        height, width = img.shape[:2]
        x1 = int(d.box.x1 * width)
        y1 = int(d.box.y1 * height)
        x2 = int(d.box.x2 * width)
        y2 = int(d.box.y2 * height)

        print(f" - {d.class_name} ({d.confidence:.2f}) at [{x1}, {y1}, {x2}, {y2}]")
